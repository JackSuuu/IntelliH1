...
Now includes MuJoCo viewer for visualization!
"""
...
class MuJoCoSimNode(Node):
    ...
    - Includes MuJoCo viewer for visual simulation (like standalone demo)
    """
    ...
    def __init__(self):
        ...
        self.declare_parameter('enable_viewer', True)  # Enable MuJoCo viewer by default
        ...
        # Initialize MuJoCo viewer (passive mode for integration with ROS2 loop)
        self.viewer = None
        if self.enable_viewer:
            ...
            try:
                ...
                self.get_logger().warn(f'Failed to launch MuJoCo viewer: {e}')
                self.viewer = None
    ...
    def timer_callback(self):
        ...
        # Sync viewer (update visualization)
...
def main(args=None):
    ...
    try:
        ...
        # Clean up viewer
        if node is not None and node.viewer is not None:
            try:
                node.viewer.close()
            except:
                pass
